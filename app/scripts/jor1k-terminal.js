...
function Terminal(rows, columns, elemId) {
    ...
    this.textarea = document.getElementById(elemId);
    //Drawing canvas stuff that's not needed
    ...
    this.ScreenUpdate();
    // Don't need blink anymore because not drawing on canvas
    //this.Blink();
}
...
Terminal.prototype.PlotRow = function(row) {
    ...
    for (var i = 0; i < this.ncolumns; i++) {
        if (ccolor != this.color[row][i]) {
            // Drawing on canvas stuff that's not needed
        ...
        if (this.screen[row][i] == 0x0) {
            ...
            //line = this.screen[row][i];
            //console.log(this.screen[row][i]);
            //console.log("B" + String.fromCharCode(10) + "A");
        }
        //if(line === '\n') console.log('rawr2');
    }
